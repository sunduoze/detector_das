...
OneButton ROTARY_Btn(ROTARY_P, true);
...
void rotary_init(void)
{
    ...
    pinMode(ROTARY_P, INPUT_PULLUP);

    // 初始化按键事件检测
    ROTARY_Btn.attachClick(sys_Counter_click);
    ROTARY_Btn.attachDoubleClick(sys_Counter_doubleclick);
    ROTARY_Btn.attachLongPressStart(sys_Counter_longclick);
    ROTARY_Btn.setDebounceTicks(35); // 25
    ROTARY_Btn.setClickTicks(150);   // 30
    ROTARY_Btn.setPressTicks(500);   // 50
...
void ICACHE_RAM_ATTR sys_Counter_IRQHandler(void)
{
    ...
    // 更新编码器方向
    double step = (rotary_dir == 0) ? Count_step : -Count_step;
